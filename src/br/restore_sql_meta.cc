...
butil::Status RestoreSqlMeta::Run() { return butil::Status::OK(); }

butil::Status RestoreSqlMeta::Finish() { return butil::Status::OK(); }
...
butil::Status RestoreSqlMeta::ExtractFromStoreRegionSqlMetaSst() {
  ...
  id_and_region_kvs_ = std::make_shared<std::map<int64_t, dingodb::pb::common::Region>>();
  for (const auto& [internal_id, region_str] : internal_id_and_region_kvs) {
    ...
    id_and_region_kvs_->emplace(std::stoll(internal_id), std::move(region));
  }
...
butil::Status RestoreSqlMeta::ExtractFromStoreCfSstMetaSqlMetaSst() {
  ...
  for (const auto& [internal_id, group_str] : internal_id_and_sst_meta_group_kvs) {
    ...
    id_and_sst_meta_group_kvs_->emplace(std::stoll(internal_id), std::move(group));
  }
