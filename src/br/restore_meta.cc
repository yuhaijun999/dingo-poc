...
butil::Status RestoreMeta::Init() {
  ...
  if (!restore_sql_meta_) {
    ...
    restore_sql_meta_ = std::make_shared<RestoreSqlMeta>(coordinator_interaction, store_interaction, restorets_,
                                                         restoretso_internal_, storage_, storage_internal_,
                                                         store_region_sql_meta_sst_, store_cf_sst_meta_sql_meta_sst_);
  }
...
butil::Status RestoreMeta::CreateAutoIncrementsToMeta() {
  ...
  auto status = coordinator_interaction_->SendRequest("MetaService", "CreateAutoIncrementsRequest", request, response);
  if (!status.ok()) {
    DINGO_LOG(ERROR) << status.error_cstr();
    return status;
  }
...
butil::Status RestoreMeta::ExtractFromBackupMeta() {
  ...
  if (iter == backupmeta_schema_kvs_.end()) {
    std::string s =
        fmt::format("not found {} in backupmeta.schema file.", dingodb::Constant::kStoreRegionSqlMetaSstName);
    DINGO_LOG(ERROR) << s;
  ...
  ret = internal_store_cf_sst_meta_sql_meta_sst.ParseFromString(iter->second);
  if (!ret) {
    std::string s = fmt::format("parse dingodb::pb::common::BackupParam failed");
    return butil::Status(dingodb::pb::error::Errno::EINTERNAL, s);
  }
