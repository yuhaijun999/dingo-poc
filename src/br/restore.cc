...
Restore::Restore(const RestoreParams& params)
    : is_gc_stop_(false),
...
butil::Status Restore::DoRun() {
  ...
  // create restore meta
  {
    ...
    restore_meta_ = std::make_shared<RestoreMeta>(coordinator_interaction, store_interaction, restorets_,
                                                  restoretso_internal_, storage_, storage_internal_, backup_meta_meta,
                                                  id_epoch_type_and_value, table_increment_group);
  }
  ...
  // create restore data
  {
    ...
    restore_data_ = std::make_shared<RestoreData>(coordinator_interaction, store_interaction, index_interaction,
                                                  document_interaction, restorets_, restoretso_internal_, storage_,
                                                  storage_internal_, backup_meta_data);
  }
...
butil::Status Restore::DoFinish() { return butil::Status::OK(); }
